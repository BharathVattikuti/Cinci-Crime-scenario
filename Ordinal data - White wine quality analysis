...
#attempted gains chart
#use actual = for real predictions, and predicted = for predicted values from any model
library("gains")

test_index <- sample(nrow(wine.test))
shuffled <- wine.test[test_index,] #use to re-sample and re-predict

gains(actual = as.numeric(shuffled$response), predicted = as.numeric(wine.rf.pred), groups = 3)
